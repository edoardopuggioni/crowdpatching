...
// Additional constructor for separating entities
// In practice this is used by the generator G
...
// Original up-to-date constructor in the jsnark library
CircuitReader::CircuitReader(char *arithFilepath, char *inputsFilepath, ProtoboardPtr pb)
{
	this->pb = pb;
	numWires = 0;
	numInputs = numNizkInputs = numOutputs = 0;
...
// Additional method for separating entities
// This method is needed for G to generate the keys without
// knowing the input values. It is used instead of the
// original method parseAndEvaluate() and it seems like the
// only differences are some commented parts with respect
// to that method. If in doubt, check the file sent by H.
void CircuitReader::parse(char *arithFilepath)
{
	libff::enter_block("Parsing (no evaluating) the circuit");
	...
	// leave_block("Parsing the circuit");
}
...
void CircuitReader::parseAndEval(char *arithFilepath, char *inputsFilepath)
{
	...
	while (getline(arithfs, line))
	{
							 ...
							 &numGateInputs, inputStr, &numGateOutputs, outputStr))
		{
			...
			{
				wireValues[outWires[0]] =
					(inValues[0] == zeroElement && inValues[1] == zeroElement) ? zeroElement : oneElement;
			}
...
int CircuitReader::find(Wire wireId, LinearCombinationPtr &lc, bool intentionToEdit)
{
...
void CircuitReader::addSplitConstraint(char *inputStr, char *outputStr, unsigned short n)
{
